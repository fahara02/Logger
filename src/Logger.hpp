...
class Logger
{
   ...
   public:
     ...
     * @param cb User callback, called with each null-terminated log line (including newline). Pass
     * nullptr to disable.
     */
 ...
 */
static inline void ENABLE_REPORT(LogReportCallback cb) { Logger::getInstance().enableReport(cb); }
/** Disable report/file logging. */
static inline void DISABLE_REPORT() { Logger::getInstance().disableReport(); }
/** Returns true if report logging is enabled. */
static inline bool IS_REPORT_ENABLED() { return Logger::getInstance().isReportEnabled(); }
